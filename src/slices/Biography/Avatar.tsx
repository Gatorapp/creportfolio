...
export default function Avatar({image,className,}: AvatarProps){
    ...
    return (
        <div ref={component} className={clsx("relative h-full w-full", className)}>
            <div className="avatar aspect-square overflow-hidden rounded-3xl boreder-2 border-slate-700 opacity-0f">
            <PrismicNextImage
            field={image}
            className="avatar-image h-full w-full object-fill"
            imgixParams={{ q: 90 }}
            />
            <div className="highlight absolute inset-0 hidden w-full scale-100 bg-radient-to-tr from -transparent via white to-transparent opacity-0 md:block"></div>
            </div>
        </div>
